  ...
  if [[ -n ${DOTFILES_INSTALL_LOG_FILE:-} ]]; then
    ...
    if [ -f "/var/log/archinstall/install.log" ]; then
      ...
      if [ -n "$ARCHINSTALL_START" ] && [ -n "$ARCHINSTALL_END" ]; then
        ...
        echo "Archinstall: ${ARCH_MINS}m ${ARCH_SECS}s" >>"$DOTFILES_INSTALL_LOG_FILE"
      fi
    fi

    if [ -n "$DOTFILES_START_TIME" ]; then
      DOTFILES_START_EPOCH=$(date -d "$DOTFILES_START_TIME" +%s)
      DOTFILES_END_EPOCH=$(date -d "$DOTFILES_END_TIME" +%s)
      DOTFILES_DURATION=$((DOTFILES_END_EPOCH - DOTFILES_START_EPOCH))
      ...
      if [ -n "$ARCH_DURATION" ]; then
        ...
        echo "Total:       ${TOTAL_MINS}m ${TOTAL_SECS}s" >>"$DOTFILES_INSTALL_LOG_FILE"
      fi
    fi
    echo "=================================" >>"$DOTFILES_INSTALL_LOG_FILE"

    echo "Rebooting system..." >>"$DOTFILES_INSTALL_LOG_FILE"
  fi
}
